...
func genClient(file *protogen.File, service *protogen.Service, g *protogen.GeneratedFile) {
	...
	for _, method := range service.Methods {
		...
		if !method.Desc.IsStreamingClient() && !method.Desc.IsStreamingServer() {
			...
			g.P(g.QualifiedGoIdent(slogPackage.Ident("ErrorContext")), "(ctx,\"[", pathurl, "] invoke failed\",", g.QualifiedGoIdent(slogPackage.Ident("String")), "(\"error\",e.Error()))")
			g.P("return nil,e")
			g.P("}")
			g.P("return resp, nil")
			g.P("}")
		...
		if !method.Desc.IsStreamingClient() && method.Desc.IsStreamingServer() {
			...
			g.P("if e!=nil{")
			g.P(g.QualifiedGoIdent(slogPackage.Ident("ErrorContext")), "(ctx,\"[", pathurl, "] create stream failed\",", g.QualifiedGoIdent(slogPackage.Ident("String")), "(\"error\",e.Error()))")
			g.P("cancel()")
			g.P("return e")
			g.P("}")
			...
			g.P(g.QualifiedGoIdent(slogPackage.Ident("ErrorContext")), "(ctx,\"[", pathurl, "] finish send request failed\",", g.QualifiedGoIdent(slogPackage.Ident("String")), "(\"error\",e.Error()))")
			g.P("cancel()")
			g.P("return e")
			g.P("}")
