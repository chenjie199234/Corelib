...
func genClient(file *protogen.File, service *protogen.Service, g *protogen.GeneratedFile) {
	...
	for _, method := range service.Methods {
		...
		if !method.Desc.IsStreamingClient() && method.Desc.IsStreamingServer() {
			...
			g.P("reqd,_:=", g.QualifiedGoIdent(protoPackage.Ident("Marshal")), "(req)")
			g.P("execute:=false")
			g.P("e := c.cc.Call(ctx,", pathname, ",reqd,", g.QualifiedGoIdent(crpcPackage.Ident("Encoder_PROTOBUF")), ",func(ctx *", g.QualifiedGoIdent(crpcPackage.Ident("CallContext")), ")error{")
			g.P("execute=true")
		...
		if method.Desc.IsStreamingClient() && method.Desc.IsStreamingServer() {
			...
			g.P("execute:=false")
			g.P("e := c.cc.Call(ctx,", pathname, ",nil,0,func(ctx *", g.QualifiedGoIdent(crpcPackage.Ident("CallContext")), ")error{")
			g.P("execute = true")
