...
func genFileComment(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile) {
	...
	g.P("// \tprotoc-gen-go-crpc ", version.String())
...
func genServer(file *protogen.File, service *protogen.Service, g *protogen.GeneratedFile) {
	...
	for _, method := range service.Methods {
		...
		if !method.Desc.IsStreamingClient() && !method.Desc.IsStreamingServer() {
			...
			g.P("reqbody,encoder,e:=ctx.Recv()")
			g.P("if e!=nil {")
			...
			g.P("if e:=", g.QualifiedGoIdent(protojsonPackage.Ident("Unmarshal")), "(reqbody,req);e!=nil{")
			g.P(g.QualifiedGoIdent(slogPackage.Ident("ErrorContext")), "(ctx,\"[", pathurl, "] request decode failed\",", g.QualifiedGoIdent(slogPackage.Ident("String")), "(\"error\",e.Error()))")
			g.P("ctx.Abort(", g.QualifiedGoIdent(cerrorPackage.Ident("ErrReq")), ")")
			g.P("return")
			g.P("}")
			g.P("default:")
			g.P(g.QualifiedGoIdent(slogPackage.Ident("ErrorContext")), "(ctx,\"[", pathurl, "] request encoder unknown\")")
			g.P("ctx.Abort(", g.QualifiedGoIdent(cerrorPackage.Ident("ErrReq")), ")")
			g.P("return")
			g.P("}")
		...
		if !method.Desc.IsStreamingClient() && method.Desc.IsStreamingServer() {
			...
			g.P("case ", g.QualifiedGoIdent(crpcPackage.Ident("Encoder_PROTOBUF")), ":")
			g.P("if e:=", g.QualifiedGoIdent(protoPackage.Ident("Unmarshal")), "(reqbody,req);e!=nil{")
			g.P(g.QualifiedGoIdent(slogPackage.Ident("ErrorContext")), "(ctx,\"[", pathurl, "] decode request failed\",", g.QualifiedGoIdent(slogPackage.Ident("String")), "(\"error\",e.Error()))")
			g.P("ctx.Abort(", g.QualifiedGoIdent(cerrorPackage.Ident("ErrReq")), ")")
			g.P("return")
			g.P("}")
			g.P("case ", g.QualifiedGoIdent(crpcPackage.Ident("Encoder_JSON")), ":")
			...
			g.P("}")
			g.P("default:")
			g.P(g.QualifiedGoIdent(slogPackage.Ident("ErrorContext")), "(ctx,\"[", pathurl, "] unknown request encoder\")")
			g.P("ctx.Abort(", g.QualifiedGoIdent(cerrorPackage.Ident("ErrReq")), ")")
			g.P("return")
			g.P("}")
			...
			g.P("if e:=handler(", g.QualifiedGoIdent(crpcPackage.Ident("NewServerStreamServerContext")), "[", g.QualifiedGoIdent(method.Output.GoIdent), "](ctx,encoder),req);e!=nil{")
			g.P("ctx.Abort(e)")
			g.P("return")
			g.P("}")
			g.P("}")
		...
		if method.Desc.IsStreamingClient() && !method.Desc.IsStreamingServer() {
			...
			g.P(fname, "(", p1, ")", freturn, "{")
			g.P("return func(ctx *", g.QualifiedGoIdent(crpcPackage.Ident("ServerContext")), "){")
			g.P("resp,e:=handler(", g.QualifiedGoIdent(crpcPackage.Ident("NewClientStreamServerContext")), "[", g.QualifiedGoIdent(method.Input.GoIdent), "](ctx,", pbex.NeedValidate(method.Input), "))")
			g.P("if e!=nil {")
			g.P("ctx.Abort(e)")
			g.P("return")
			g.P("}")
		...
		if method.Desc.IsStreamingClient() && method.Desc.IsStreamingServer() {
			...
			g.P(fname, "(", p1, ")", freturn, "{")
			g.P("return func(ctx *", g.QualifiedGoIdent(crpcPackage.Ident("ServerContext")), "){")
			g.P("e:=handler(", g.QualifiedGoIdent(crpcPackage.Ident("NewAllStreamServerContext")), "[", g.QualifiedGoIdent(method.Input.GoIdent), ",", g.QualifiedGoIdent(method.Output.GoIdent), "](ctx,", pbex.NeedValidate(method.Input), "))")
			g.P("if e!=nil {")
			g.P("ctx.Abort(e)")
			g.P("}")
			g.P("}")
...
func genClient(file *protogen.File, service *protogen.Service, g *protogen.GeneratedFile) {
	...
	for _, method := range service.Methods {
		...
		if !method.Desc.IsStreamingClient() && !method.Desc.IsStreamingServer() {
			...
			g.P("if e:=c.cc.Call(ctx,", pathname, ",reqd,", g.QualifiedGoIdent(crpcPackage.Ident("Encoder_PROTOBUF")), ",func(ctx *", g.QualifiedGoIdent(crpcPackage.Ident("CallContext")), ")error{")
			g.P("var err error")
			g.P("if respbody,encoder,err = ctx.Recv();err!=nil{")
			g.P(g.QualifiedGoIdent(slogPackage.Ident("ErrorContext")), "(ctx,\"[", pathurl, "] read response failed\",", g.QualifiedGoIdent(slogPackage.Ident("String")), "(\"error\",err.Error()))")
			g.P("}")
			g.P("return err")
			g.P("});e!=nil{")
			// g.P(g.QualifiedGoIdent(slogPackage.Ident("ErrorContext")), "(ctx,\"[", pathurl, "] execute request failed\",", g.QualifiedGoIdent(slogPackage.Ident("String")), "(\"error\",e.Error()))")
			g.P("return nil,e")
			g.P("}")
			g.P("resp := new(", g.QualifiedGoIdent(method.Output.GoIdent), ")")
			...
			g.P("}")
			g.P("case ", g.QualifiedGoIdent(crpcPackage.Ident("Encoder_JSON")), ":")
			g.P("if e:=", g.QualifiedGoIdent(protojsonPackage.Ident("Unmarshal")), "(respbody,resp);e!=nil{")
			g.P(g.QualifiedGoIdent(slogPackage.Ident("ErrorContext")), "(ctx,\"[", pathurl, "] decode response failed\",", g.QualifiedGoIdent(slogPackage.Ident("String")), "(\"error\",e.Error()))")
			g.P("return nil,", g.QualifiedGoIdent(cerrorPackage.Ident("ErrResp")))
			g.P("}")
			g.P("default:")
			g.P(g.QualifiedGoIdent(slogPackage.Ident("ErrorContext")), "(ctx,\"[", pathurl, "] unknown response encoder\")")
			g.P("return nil,", g.QualifiedGoIdent(cerrorPackage.Ident("ErrResp")))
			g.P("}")
			g.P("return resp, nil")
			g.P("}")
		...
		if !method.Desc.IsStreamingClient() && method.Desc.IsStreamingServer() {
			...
			g.P("reqd,_:=", g.QualifiedGoIdent(protoPackage.Ident("Marshal")), "(req)")
			g.P("return c.cc.Call(ctx,", pathname, ",reqd,", g.QualifiedGoIdent(crpcPackage.Ident("Encoder_PROTOBUF")), ",func(ctx *", g.QualifiedGoIdent(crpcPackage.Ident("CallContext")), ")error{")
			g.P("return handler(", g.QualifiedGoIdent(crpcPackage.Ident("NewServerStreamClientContext")), "[", g.QualifiedGoIdent(method.Output.GoIdent), "](ctx))")
			g.P("})")
			g.P("}")
		}
		if method.Desc.IsStreamingClient() && !method.Desc.IsStreamingServer() {
			...
			g.P("}")
			g.P("ctx.StopSend()")
			g.P("if respbody,encoder,e = ctx.Recv();e!=nil{")
