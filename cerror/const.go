...
var (
	...
	ErrAuth           = MakeCError(10004, http.StatusUnauthorized, "auth wrong")
	ErrToken          = MakeCError(10005, http.StatusUnauthorized, "token wrong")
	ErrSession        = MakeCError(10006, http.StatusUnauthorized, "session wrong")
	ErrAccessKey      = MakeCError(10007, http.StatusUnauthorized, "access key wrong")
	ErrAccessSign     = MakeCError(10008, http.StatusUnauthorized, "access sign wrong")
	ErrPermission     = MakeCError(10009, http.StatusForbidden, "permission denie")
	ErrTooFast        = MakeCError(10010, http.StatusForbidden, "too fast")
	ErrBan            = MakeCError(10011, http.StatusForbidden, "ban")
	ErrBusy           = MakeCError(10012, http.StatusServiceUnavailable, "busy")
	ErrNotExist       = MakeCError(10013, http.StatusNotFound, "not exist")
	ErrAlreadyExist   = MakeCError(10014, http.StatusBadRequest, "already exist")
	ErrPasswordWrong  = MakeCError(10015, http.StatusBadRequest, "password wrong")
	ErrPasswordLength = MakeCError(10016, http.StatusBadRequest, "password length must <=32")
)
